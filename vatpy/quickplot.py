...
class QuickPlot:
    ...
    QuickPlot
    '''
    def __init__(self, f, save=os.getcwd(), vmin=None, vmax=None, xlim=None, ylim=None, style=None, sformat=None):
        # Variables:
        self.f       = f
        self.save    = save
        self.vmin    = vmin
    ...
    def read_hdf5(self, file):
        '''
        Description: reads given HDF5-file and returns a h5py-object 
        with a directory of internal units (in cgs)
        '''
        # Read hdf5-file:
        h = h5py.File(file, 'r')
    ...
    def number_density(self, h, iu):
        '''
        Description: calculates the total number density in gas cells, as well as 
        the number density of different chemical species
        '''
        # Gas density:  
    ...
    def density_frame(self, file, unit='cgs', zslice=None, bins=100):   
        '''
        Description: 2D interpolation of the gas density
        '''
        # Read data:
        h, iu = self.read_hdf5(file=file)
        Coord = h['PartType0']['Coordinates'][:] * iu['ulength'] / self.kpc
        ...
        # Slice at z:
        box = h['Header'].attrs['BoxSize'] * iu['ulength'] / self.kpc
        x, dx = np.linspace(0, box, bins, retstep=True)
    ...
    def density_plot(self, unit='cgs', zslice=None, bins=100):
        ...
        '''
        # Density frame:
        interp, clabel, time, box = self.density_frame(file=self.f, unit=unit, zslice=zslice, bins=bins)

        # Plot:
        fig, ax = plt.subplots(figsize=(6, 5))
        im = ax.imshow(np.log10(interp), vmin=self.vmin, vmax=self.vmax, extent=(0, box, 0, box), origin='lower', cmap='magma')
        ...
        print(f'\n#### savefig : \'{self.save + figname}\' ####\n')
        
        return 0

    def movie(self, unit='cgs', zslice=None, bins=100):
        '''
        Description: animation of how the time evolution of the gas density 
        '''
        # Number of snapshots:
        ...
        for i in range(0, N+1):
            ...
            interp, clabel, time, box = self.density_frame(file=snap, unit=unit, zslice=zslice, bins=bins)
